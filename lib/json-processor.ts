  ...
  const extractSegmentsFromSource = async (sourceData: RawJsonData, sourceName: string) => {
    ...
    for (let geoIdx = 0; geoIdx < geographies.length; geoIdx++) {
      ...
      if (sourceData[geography]?.[segmentType]) {
        ...
        // For "By Region" segment type, build hierarchy for each region found
        if (isRegionSegmentType && geography !== 'Global') {
          // The JSON structure is: Geography > "By Region" > Region > Country
          // e.g., Europe > By Region > Europe > U.K.
          //       Europe > By Region > North America > U.S.
          // We need to build hierarchy for EACH region, not just the parent geography
          // hierarchy["Europe"] = ["U.K.", "Germany", ...]
          // hierarchy["North America"] = ["U.S."]
          // hierarchy["Asia Pacific"] = ["China", ...]

          structurePaths.forEach(({ path: pathArray }) => {
            const segmentPath = pathArray.slice(segmentTypeIndex + 1)

            // segmentPath structure: [RegionName, CountryName]
            // e.g., ["Europe", "U.K."] or ["North America", "U.S."]
            if (segmentPath.length >= 2) {
              const regionName = segmentPath[0]
              const countryName = segmentPath[1]

              if (regionName && regionName.trim() !== '' && countryName && countryName.trim() !== '') {
                // Add region as a segment item (root level)
                if (!segmentItems.includes(regionName)) {
                  segmentItems.push(regionName)
                }

                // Create hierarchy entry for this region if not exists
                if (!hierarchy[regionName]) {
                  hierarchy[regionName] = []
                }

                // Add country as child of region (not as child of geography)
                if (!hierarchy[regionName].includes(countryName)) {
                  hierarchy[regionName].push(countryName)
                }
              }
            } else if (segmentPath.length === 1 && segmentPath[0] && segmentPath[0].trim() !== '') {
              // Single level - this is a region without sub-countries (leaf region)
              const regionName = segmentPath[0]
              if (!segmentItems.includes(regionName)) {
                segmentItems.push(regionName)
              }
              // Create self-referencing hierarchy for leaf regions
              if (!hierarchy[regionName]) {
                hierarchy[regionName] = [regionName]
              }
            }
          })
...
export async function processJsonDataAsync(
  ...
  try {
    ...
    // Extract regions from "By Region" segment type and add them to geography dimension
    // This ensures geography mode dropdown shows regions like "North America", "Europe", etc.
    const byRegionSegment = segments['By Region']
    if (byRegionSegment && byRegionSegment.hierarchy) {
      const regionNames = Object.keys(byRegionSegment.hierarchy)
      console.log('Extracting regions from "By Region" segment type:', regionNames)

      // Add regions to all_geographies if they don't already exist
      regionNames.forEach(region => {
        if (!geographyDimension.all_geographies.includes(region)) {
          geographyDimension.all_geographies.push(region)
        }
        // Also add to regions array
        if (!geographyDimension.regions.includes(region)) {
          geographyDimension.regions.push(region)
        }
      })

      // Also add country-level geographies from hierarchy
      Object.entries(byRegionSegment.hierarchy).forEach(([region, countries]) => {
        if (Array.isArray(countries)) {
          countries.forEach(country => {
            // Skip self-referencing entries (e.g., "Europe" under "Europe")
            if (country !== region && !geographyDimension.all_geographies.includes(country)) {
              geographyDimension.all_geographies.push(country)
            }
          })
          // Build countries mapping
          if (!geographyDimension.countries[region]) {
            geographyDimension.countries[region] = []
          }
          countries.forEach(country => {
            if (country !== region && !geographyDimension.countries[region].includes(country)) {
              geographyDimension.countries[region].push(country)
            }
          })
        }
      })

      console.log(`Geography dimension updated with regions. Total geographies: ${geographyDimension.all_geographies.length}`)
    }
    ...
    const metadata: Metadata = {
      market_name: 'Global Employee Advocacy Tool Market',
