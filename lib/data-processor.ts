...
export function filterData(
  ...
  const filtered = data.filter((record) => {
    ...
    if (!geoMatch) {
      ...
      // IMPORTANT: Include Global data as fallback when:
      // 1. User selected regional geographies (North America, Europe, Asia, etc.)
      // 2. No direct records exist for those geographies with this segment type
      // 3. In Geography Mode: Always include Global so it can be distributed to selected regions
      // 4. In Segment Mode: Include Global as fallback data source
      if (!geoMatch && record.geography === 'Global') {
        // All regional geographies that should trigger Global fallback
        const allRegionalGeographies = [
          // Old hierarchy
          'North America', 'Asia Pacific', 'Latin America', 'Middle East', 'Africa',
          // New hierarchy
          'Europe', 'Asia',
          // Countries (in case user selects them directly)
          'Germany', 'Italy', 'UK', 'Rest of Europe', 'Japan', 'South Korea', 'China', 'Rest of Asia',
          'U.S.', 'United States', 'Canada'
        ]
        const hasRegionalSelection = filters.geographies.some(g => allRegionalGeographies.includes(g))
        const hasGlobalSelected = filters.geographies.includes('Global')

        // Include Global when regional geographies are selected (but Global itself is not selected)
        // Downstream functions will distribute Global data to regional geographies
        if (hasRegionalSelection && !hasGlobalSelected) {
          // Geography Mode: prepareIntelligentMultiLevelData will distribute Global data to regions
          // Segment Mode: Global data serves as fallback for segment types without regional data
          // Matrix Mode: MatrixHeatmap will distribute Global data to selected regional geographies
          geoMatch = true
        }
      }
    }
      ...
      if (record.is_aggregated === true) {
        ...
        if (selectedLevel1Segments.length > 0) {
          ...
          if (isRegionalSegmentType) {
            ...
            // Each selection can be either a region name or a country name
            let matchesAnySelection = false

            for (const selectedSeg of selectedLevel1Segments) {
              // For "By Region" segment type, always check segment hierarchy first
              // because regions (North America, Europe) are stored in segment_hierarchy.level_1
              // and countries (U.S., Germany) are stored in segment or segment_hierarchy.level_2
              const hierarchy = record.segment_hierarchy

              // Check if selected segment matches hierarchy level_1 (region names like "North America", "Europe")
              if (hierarchy.level_1 === selectedSeg) {
                matchesAnySelection = true
                break
              }

              // Check if selected segment matches the record's segment directly (country names)
              if (record.segment === selectedSeg) {
                matchesAnySelection = true
                break
              }

              // Check hierarchy level_2 for country matches
              if (hierarchy.level_2 === selectedSeg) {
                matchesAnySelection = true
                break
              }

              // Fallback: Check if this is a geography-level selection (legacy support)
              if (regionalGeographies.includes(selectedSeg) && record.geography === selectedSeg) {
                matchesAnySelection = true
                break
              }
            }
    ...
    if (filters.advancedSegments && filters.advancedSegments.length > 0) {
      ...
      if (effectiveAggregationLevel === 1) {
        ...
        segmentMatch = filters.advancedSegments.some(seg => {
          ...
          // For "By Region" segment type, regions (North America, Europe) are in segment_hierarchy.level_1
          const hierarchy = record.segment_hierarchy
          ...
          // Special handling for "By Region" - also check geography as fallback (legacy support)
          // For new data, regions are in hierarchy; for legacy data, they might be in geography
          if (isRegionSegmentType && seg.segment === record.geography) {
            console.log('🔍 SEGMENT FILTER: By Region geography match (legacy):', {
    ...
    records.forEach(record => {
      ...
      records.forEach(record => {
        ...
        if (isLevel1) {
          ...
          // For "By Region" data, geography is typically "Global" and regions are in segment_hierarchy.level_1
          const hierarchy = record.segment_hierarchy
          ...
          for (const selectedSeg of selectedSegmentNames) {
            // PRIORITY 1: Check segment_hierarchy.level_1 (region names like "North America", "Europe")
            // This is where region names are stored for "By Region" segment type
            if (hierarchy.level_1 === selectedSeg) {
              matchedSelection = selectedSeg
              break
            }

            // PRIORITY 2: Check direct segment match (country names)
            if (record.segment === selectedSeg) {
              matchedSelection = selectedSeg
              break
            }

            // PRIORITY 3: Check hierarchy level_2 for country matches
            if (hierarchy.level_2 === selectedSeg) {
              matchedSelection = selectedSeg
              break
            }

            // FALLBACK: Check geography field (for legacy data)
...
export function prepareIntelligentMultiLevelData(
  ...
  records.forEach(record => {
    ...
    // 1. Region names (e.g., North America, Europe) - stored in segment_hierarchy.level_1
    // 2. Country/state names (e.g., U.S., Canada, Germany) - stored in segment or segment_hierarchy.level_2
    // The geography field is typically "Global" for By Region data
    if (isRegionalSegmentType && hasExplicitLevel1Selection) {
      ...
      for (const selectedSeg of selectedLevel1Segments) {
        ...
        // FALLBACK: Check geography field (for legacy data where geography might be the region)
        if (record.geography === selectedSeg) {
          matchedSelection = selectedSeg
          break
        }
