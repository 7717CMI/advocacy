...
function OpportunityGeographyMultiSelect() {
  ...
  const geographyOptions = useMemo(() => {
    ...
    // Use all geographies from the data dynamically (no hardcoded filtering)
    // The data's top-level geographies are the root geographies (U.S., Europe, Asia Pacific)
  ...
  const chartData = useMemo(() => {
    ...
    if (isOpportunityMode) {
      ...
      filteredRecords.forEach((record, index) => {
        ...
        // Avoid duplicate labels like "Europe - Europe" when geography equals segment name
        let bubbleName: string
        if (segmentName && segmentName !== '__ALL_SEGMENTS__') {
          // If geography and segment name are the same (e.g., "By Region" where region = geography)
          // just use geography name to avoid "Europe - Europe"
          if (record.geography === segmentName) {
            bubbleName = record.geography
          } else {
            bubbleName = `${record.geography} - ${segmentName}`
          }
        } else {
          bubbleName = `${record.geography} - ${record.segment_type}`
        }
