...
export function CascadeFilter({
  ...
  const getLevelOptions = (level: number): LevelOption[] => {
    ...
    if (level === 0) {
      ...
      // Filter out parent regions that have only one child which is already a root
      // e.g., if "North America" → ["U.S."] and "U.S." is already a root, remove "North America"
      const filteredRoots = roots.filter(root => {
        const children = hierarchy[root]
        if (!children || !Array.isArray(children) || children.length !== 1) {
          return true // Keep roots with 0 or 2+ children
        }
        const onlyChild = children[0]
        // If the only child is the same as root (self-referencing), keep it
        if (onlyChild === root) {
          return true
        }
        // If the only child is also a root, remove this parent (it's redundant)
        if (roots.includes(onlyChild)) {
          return false
        }
        return true
      })
      ...
      const finalRoots = filteredRoots.length > 0 ? filteredRoots : Object.keys(hierarchy)
